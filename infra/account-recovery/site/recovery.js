...
(() => {
  ...
  async function post(path, body) {
    ...
    try {
      ...
      if (!response.ok) {
        let code;
        // Do not display arbitrary server/proxy bodies, which could echo secrets.
        ...
        if (reader) {
          ...
          while (true) {
            ...
            if (done) break;
            if (bytes.length + value.length > 8192) { await reader.cancel(); break; }
            const next = new Uint8Array(bytes.length+value.length);next.set(bytes);next.set(value,bytes.length);bytes=next;
          }
          try { code = JSON.parse(new TextDecoder().decode(bytes)).code; } catch (_) {}
        }
        throw new Error(known[code] || (response.status === 429 ? known.recovery_rate_limited : "Couldn't complete this request. Please try again later."));
      }
    } catch (error) {
  ...
  const setup = async () => {
    if (mode === "forgot-password") {
      ...
      if (mode === "reset-password") {
        ...
        await post("/v2/accounts/password-reset/inspect", {token});
