...
cp "$repo_dir/tests/quickshell/AnchoredPickers.qml" "$test_dir/shell.qml"
...
for theme in ${WISP_TEST_THEMES:-soft_graphite daylight hearth clean_tui}; do
  ...
  if rg 'PICKERS_FAILED|TypeError|ReferenceError|Binding loop|Cannot assign|Cannot anchor|Failed to load' "$test_dir/log"; then cat "$test_dir/log"; exit 1; fi
  rg -q PICKERS_OK "$test_dir/log" || { cat "$test_dir/log"; exit 1; }
done
echo 'Anchored emoji and invite picker bounds passed in all requested themes'
