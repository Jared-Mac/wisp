...
cp "$repo_dir/tests/quickshell/RoomFlow.qml" "$test_dir/shell.qml"
...
if rg 'ROOM_FLOW_FAILED|TypeError|ReferenceError|Binding loop|Cannot assign|Cannot anchor|Failed to load' "$test_dir/log"; then cat "$test_dir/log"; exit 1; fi
rg -q ROOM_FLOW_OK "$test_dir/log" || { cat "$test_dir/log"; exit 1; }
echo 'Stable room navigation, explicit voice joining and persistent server-scoped call controls passed'
