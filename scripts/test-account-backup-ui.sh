...
for theme in ${WISP_TEST_THEMES:-soft_graphite daylight hearth clean_tui}; do
  ...
  if rg 'ACCOUNT_BACKUP_FAILED|TypeError|ReferenceError|Binding loop|Cannot assign|Cannot anchor|Failed to load' "$test_dir/log"; then cat "$test_dir/log"; exit 1; fi
  rg -q ACCOUNT_BACKUP_OK "$test_dir/log" || { cat "$test_dir/log"; exit 1; }
done
echo 'Native account backup settings passed in all themes'
