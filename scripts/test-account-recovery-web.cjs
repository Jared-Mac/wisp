...
async function page(mode,{token='A'.repeat(43),failure=null,secure=false}={}) {
 const fields={};
 ...
 fetch:async(path,options)=>{requests.push({path,options,body:JSON.parse(options.body)});return {ok:!failure,status:failure?400:200,body:{getReader(){let done=false;const data=failure || (path.endsWith('/inspect') ? {valid:true,secure_reset_required:secure} : {ok:true});return {read:async()=>done?{done:true}:(done=true,{done:false,value:Buffer.from(JSON.stringify(data))}),cancel:async()=>{}};}}};}};
...
(async()=>{
 ...
 p=await page('reset-password',{secure:true});assert.equal(p.fields['native-reset'].hidden,false);assert.equal(p.fields.form.hidden,true);assert.equal(p.fields['password-fields'].hidden,true);await p.submit();assert.equal(p.requests.length,1,'secure password never sent through browser completion');
 const html=fs.readFileSync(require('node:path').join(__dirname,'../infra/account-recovery/site/index.html'),'utf8');assert.match(html,/href="wisp:\/\/account\/reset"/);assert(!html.includes('wisp://account/reset#'));
