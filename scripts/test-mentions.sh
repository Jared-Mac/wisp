...
cp "$repo_dir/tests/quickshell/Mentions.qml" "$test_dir/shell.qml"
...
if rg 'MENTIONS_FAILED|TypeError|ReferenceError|Binding loop|Cannot assign|Failed to load' "$test_dir/log"; then cat "$test_dir/log"; exit 1; fi
rg -q MENTIONS_OK "$test_dir/log" || { cat "$test_dir/log"; exit 1; }
echo 'Mention autocomplete, rendering, encrypted-text compatibility and scoped notification filtering passed'
