...
host_config="$config_dir/host.env"
mkdir -p "$config_dir"
if [[ -f "$host_config" ]]; then
  # This file is generated by Wisp, owned by the current user, and mode 0600.
  # shellcheck disable=SC1090
  source "$host_config"
fi
WISP_BOOTSTRAP_TOKEN=${WISP_BOOTSTRAP_TOKEN:-$(openssl rand -hex 32)}
WISP_E2EE_KEY=${WISP_E2EE_KEY:-$(openssl rand -hex 32)}
temporary=$(mktemp "$config_dir/host.env.XXXXXX")
chmod 0600 "$temporary"
printf 'WISP_BOOTSTRAP_TOKEN=%s\nWISP_E2EE_KEY=%s\n' \
  "$WISP_BOOTSTRAP_TOKEN" "$WISP_E2EE_KEY" >"$temporary"
if [[ -n ${WISP_DEVICE_ID:-} && -n ${WISP_DEVICE_TOKEN:-} ]]; then
  printf 'WISP_DEVICE_ID=%s\nWISP_DEVICE_TOKEN=%s\n' \
    "$WISP_DEVICE_ID" "$WISP_DEVICE_TOKEN" >>"$temporary"
fi
mv -f -- "$temporary" "$host_config"
export WISP_BOOTSTRAP_TOKEN WISP_E2EE_KEY
export WISP_HOST_CONFIG_FILE="$host_config"
if [[ -n ${WISP_DEVICE_ID:-} && -n ${WISP_DEVICE_TOKEN:-} ]]; then
  export WISP_DEVICE_ID WISP_DEVICE_TOKEN
else
  unset WISP_DEVICE_ID WISP_DEVICE_TOKEN
fi
...
echo "Friends enroll with: just friend-register $friend_host Tyler"
echo "Create a one-use friend invite with: wispctl invite Tyler"
