...
cp "$repo_dir/tests/quickshell/ChatExtras.qml" "$test_dir/shell.qml"
...
if rg 'CHAT_EXTRAS_FAILED|TypeError|ReferenceError|Binding loop|Cannot assign|Failed to load' "$test_dir/log"; then cat "$test_dir/log"; exit 1; fi
rg -q CHAT_EXTRAS_OK "$test_dir/log" || { cat "$test_dir/log"; exit 1; }
echo 'Links, emoji insertion, scoped reactions and stable video delegates passed'
