...
cp "$repo_dir/tests/quickshell/PeopleNavigation.qml" "$test_dir/shell.qml"
...
for theme in ${WISP_TEST_THEMES:-soft_graphite daylight hearth clean_tui}; do
  ...
  if rg 'PEOPLE_NAV_FAILED|TypeError|ReferenceError|Binding loop|Cannot assign|Cannot anchor|Failed to load' "$test_dir/log"; then cat "$test_dir/log"; exit 1; fi
  rg -q PEOPLE_NAV_OK "$test_dir/log" || { cat "$test_dir/log"; exit 1; }
done
echo 'Friends/server navigation, nonmember contacts, requests, DMs and independent tray state passed'
