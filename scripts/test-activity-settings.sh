...
cp "$repo_dir/tests/quickshell/ActivitySettings.qml" "$test_dir/shell.qml"
...
for theme in ${WISP_TEST_THEMES:-soft_graphite daylight hearth clean_tui}; do
  ...
  if rg 'ACTIVITY_SETTINGS_FAILED|TypeError|ReferenceError|Binding loop|Cannot assign|Cannot anchor|Failed to load' "$test_dir/log"; then cat "$test_dir/log"; exit 1; fi
  rg -q ACTIVITY_SETTINGS_OK "$test_dir/log" || { cat "$test_dir/log"; exit 1; }
done
echo 'Activity settings defaults, independent controls, save/error handling and phone fallback passed in all themes'
