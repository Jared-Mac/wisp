...
cp "$repo_dir/tests/quickshell/MessageActions.qml" "$test_dir/shell.qml"
...
if rg 'MESSAGE_ACTIONS_FAILED|TypeError|ReferenceError|Binding loop|Cannot assign|Failed to load' "$test_dir/log"; then cat "$test_dir/log"; exit 1; fi
rg -q MESSAGE_ACTIONS_OK "$test_dir/log" || { cat "$test_dir/log"; exit 1; }
echo 'Reply drafts, forwarding confirmation, pins, permissions and old-message navigation passed'
