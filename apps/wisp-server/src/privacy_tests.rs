...
async fn setup() -> (AppState, Router, String, Identity, Identity, String) {
    ...
    let conversation = find_or_create_direct(
        &state.pool,
        JARED_ID.parse().unwrap(),
        TYLER_ID.parse().unwrap(),
    )
    .await
    .unwrap();
...
fn sealed(
    ...
    let id = Uuid::new_v4();
    let context = format!("test-network/{conversation}/{JARED_ID}/{id}/message");
    wisp_protocol::EncryptedMessageRequest {
...
async fn encrypted_message_round_trip_retry_edit_and_authorization() {
    ...
    let first =
        value(request(&app, "POST", "/v1/e2ee/messages", JARED_ID, body.clone()).await).await;
    let retry =
        value(request(&app, "POST", "/v1/e2ee/messages", JARED_ID, body.clone()).await).await;
    assert_eq!(first, retry);
    assert_eq!(
        request(&app, "POST", "/v1/e2ee/messages", CHARLIE_ID, body.clone())
            .await
            .status(),
        StatusCode::FORBIDDEN
    );
    assert_eq!(
        request(&app, "POST", "/v1/e2ee/messages", TYLER_ID, body)
            .await
            .status(),
        StatusCode::CONFLICT
    );
    let context = format!(
        "test-network/{conversation}/{JARED_ID}/{}/message",
        original.id
    );
    ...
    assert_eq!(
        request(&app, "PUT", &path, TYLER_ID, json!(edit))
            .await
            .status(),
        StatusCode::FORBIDDEN
    );
    assert_eq!(
        request(&app, "PUT", &path, JARED_ID, json!(edit))
            .await
            .status(),
        StatusCode::OK
    );
    let rows = load_recent_messages(&state.pool, TYLER_ID.parse().unwrap())
        .await
        .unwrap();
...
async fn plaintext_routes_and_fake_encryption_versions_fail_closed() {
    ...
    for (path, body) in [
        ...
        assert_eq!(
            request(&app, "POST", path, JARED_ID, body).await.status(),
            StatusCode::FORBIDDEN,
            "{path}"
        );
    }
    assert_eq!(request(&app, "POST", "/v1/e2ee/messages", JARED_ID, json!({"id":Uuid::new_v4(),"conversation_id":conversation,"ciphertext":"not an envelope"})).await.status(), StatusCode::BAD_REQUEST);
    assert!(
    ...
    sqlx::query("INSERT INTO messages(id,conversation_id,sender_id,created_at,content_type,payload,encryption_version) VALUES (?,?,?,?, 'text/plain', '\"old history\"',0)")
        .bind(Uuid::new_v4().to_string()).bind(conversation).bind(JARED_ID).bind(Utc::now().to_rfc3339()).execute(&state.pool).await.unwrap();
    assert!(
        privacy::ensure_ciphertext_storage(&state.pool)
            .await
            .is_err()
...
async fn encrypted_attachment_chunks_keep_and_expiry_work_without_filenames() {
    ...
    assert_eq!(
        request(&app, "POST", "/v1/e2ee/file-uploads", JARED_ID, begin)
            .await
            .status(),
        StatusCode::OK
    );
    assert_eq!(request(&app,"POST","/v1/file-uploads",JARED_ID,json!({"id":upload,"conversation_id":conversation,"size":bytes.len(),"file_name":"Encrypted attachment","caption":"plaintext"})).await.status(),StatusCode::FORBIDDEN);
    ...
    for _ in 0..2 {
        let mut req = Request::builder()
            ...
            .unwrap();
        *req.headers_mut() = chat_headers(JARED_ID);
        assert_eq!(
    ...
    assert_eq!(
        request(&app, "PATCH", &retention, TYLER_ID, json!({"keep":true}))
            .await
            .status(),
        StatusCode::OK
    );
