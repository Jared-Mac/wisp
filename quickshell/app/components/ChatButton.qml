...
Button {
  ...
  implicitWidth: label.implicitWidth + theme.space(theme.comfortable ? 20 : theme.tui ? 12 : 24)
  background: Rectangle {
    ...
    border.width: root.theme.comfortable ? (root.quiet && !root.visualFocus ? 0 : 1) : root.theme.tui ? (root.visualFocus ? 1 : 0) : root.theme.terminal ? 1 : 0
  ...
  contentItem: Text {
    id: label
    text: root.theme.tui && !root.theme.comfortable ? "[" + (["···", "⋯", "…"].indexOf(root.text) >= 0 ? ":" : root.text) + "]" : root.text
    color: (root.theme.comfortable && root.primary) || (root.theme.tui && (root.primary || root.down)) ? root.theme.selectionText : root.primary ? (root.destructive ? "#151821" : root.theme.terminal ? root.theme.accent : root.theme.accentText) : root.destructive ? root.theme.danger : root.theme.cleanTui && !root.theme.comfortable ? (root.hovered ? root.theme.foreground : root.theme.muted) : root.theme.foreground
