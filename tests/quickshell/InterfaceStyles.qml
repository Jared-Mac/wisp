...
ShellRoot {
  ...
  Timer {
    ...
    onTriggered: {
      test.check(appearance.profile === "legacy" && appearance.palette === "wisp", "fresh desktop preserves the Classic default")
      ...
      for (var style of ["soft_graphite","daylight","hearth"]) {
        ...
        for (var w of [Number(Quickshell.env("WISP_TEST_WIDTH")) || 1180]) {
          ...
          var serverPeople = test.find(friends,"serverPeopleSection")
          test.check(!!serverPeople && !!test.find(serverPeople,"sidebarServerMembers"), "Other members live below friends at " + style + "/" + w)
