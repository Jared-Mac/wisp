...
ShellRoot {
  ...
  Timer {
    ...
    onTriggered: {
      ...
      search.text = "video codec"; search.accepted(); input.wait(120)
      var codec = test.find(page,"settingsVideoCodec")
      test.check(menu.section === "video" && codec.visible, "search expands advanced video controls")
      test.find(page,"advancedVideoButton").clicked(); input.wait(60)
      test.check(!codec.visible, "advanced video controls can be collapsed")
      search.text = "test my voice"; search.accepted(); input.wait(120)
      test.check(menu.section === "media" && menu.revealedTarget === "settingsVoiceTest", "voice test search opens Audio")
      ...
      for (var entry of Search.entries) {
        if (entry.section === "server" && !bridge.canManageServer || entry.panel && !menu.anchorController) continue
        menu.openSearchResult(entry); input.wait(40)
        test.check(!!menu.findSetting(menu,entry.target),"search loads its target: " + entry.target)
      }
