...
ShellRoot {
  ...
  Component.onCompleted:{
    ...
    var message={id:"22222222-2222-4222-8222-222222222222",sender:{id:"friend",display_name:"Friend"},conversation_id:"chat",created_at:new Date().toISOString(),content_type:"text/plain",payload:'<b>plain text</b> https://example.com/path?q=1&b=2 :wisp_love: :wisp_coffee: 😀\nhttps://youtu.be/aqz-KE-bpKQ',encryption_version:0}
  ...
  Timer {
    ...
    onTriggered:{
      ...
      test.check(body && body.text.indexOf("assets/emojis/coffee.svg")>=0,"Everyday emoji renders inline")
      ...
      bridge.chatExtras.reply({ok:true},{action:"reaction",epoch:bridge.chatExtras.epoch,key:bridge.chatExtras.key("local",id)})
      ...
      // Exercise the real picker, resolver and SVG loader for the additional set.
      composer.emojiPicker.open();input.wait(60)
      var emojiSearch=test.find(composer.emojiPicker.contentItem,"emojiSearch")
      emojiSearch.text="everyday";input.wait(60)
      test.check(composer.emojiPicker.options.length===12,"Everyday search isolates the additional set")
      emojiSearch.text="coffee";input.wait(40)
      test.check(composer.emojiPicker.options.length===1 && composer.emojiPicker.options[0].emoji===":wisp_coffee:","descriptive keyword search")
      Markup.wispEveryday.concat(Markup.wispMoments).forEach(function(item){
        emojiSearch.text="wisp_"+item.name;input.wait(60)
        var option=test.find(composer.emojiPicker.contentItem,"emojiChoice-wisp_"+item.name)
        var artwork=test.find(option,"emojiImage-wisp_"+item.name)
        test.check(option && artwork && artwork.status===Image.Ready,"Everyday artwork loads: "+item.name)
        if(option)option.clicked()
        test.check(bridge.draftFor("local::chat").indexOf(":wisp_"+item.name+":")>=0,"Everyday insertion: "+item.name)
        composer.emojiPicker.open();input.wait(30)
      })
      composer.emojiPicker.close()
      var newReaction=JSON.parse(JSON.stringify(bridge.snapshot))
      newReaction.server_states[0].reactions.push({target_id:id,message:{id:"everyday-reaction",payload:{target:id,emoji:":wisp_coffee:"},sender:{id:"friend",display_name:"Friend"},content_type:"application/vnd.wisp.reaction+json"}})
      bridge.applySnapshot(newReaction);input.wait(60)
      var everydayReaction=test.find(feed,"reaction-"+id+"-:wisp_coffee:")
      test.check(!!everydayReaction,"Everyday reaction chip renders")
      if(everydayReaction)everydayReaction.clicked()
      var everydayCommand=bridge.sent[bridge.sent.length-1]
      test.check(everydayCommand.name==="toggle_reaction" && everydayCommand.args.emoji===":wisp_coffee:" && everydayCommand.args.server_id==="local" && everydayCommand.args.message_id===id,"Everyday reaction preserves shortcode and routing")
