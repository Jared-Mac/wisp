...
ShellRoot {
  ...
  Timer {
    ...
    onTriggered: {
      ...
      var privacy = test.find(manager.contentItem,"roomInviteOnly")
      test.check(privacy && privacy.enabled && !privacy.checked,"ordinary room defaults public and admin can change privacy")
      input.mouseClick(privacy,privacy.width/2,privacy.height/2); input.wait(40)
      test.check(test.last().name === "rename_server_room" && test.last().args.private === true && test.last().args.id === "spot:lounge" && test.last().args.server_id === "local","privacy change targets the room on the correct server")
      test.reply({ok:true})
      var memberSnapshot=JSON.parse(JSON.stringify(bridge.snapshot));memberSnapshot.server_states[0].self.server_owner=false;memberSnapshot.server_states[0].self.server_admin=false
      bridge.applySnapshot(memberSnapshot);input.wait(40)
      test.check(!privacy.enabled,"ordinary members cannot change room privacy")
      memberSnapshot=JSON.parse(JSON.stringify(bridge.snapshot));memberSnapshot.server_states[0].self.server_owner=true;bridge.applySnapshot(memberSnapshot);input.wait(40)
      manager.close(); input.wait(80)
      manager.createRoom(); input.wait(40)
      test.check(privacy.visible && privacy.enabled && !privacy.checked,"admins can choose privacy before creating a room")
      var beforeCreate=bridge.sent.length
      input.mouseClick(privacy,privacy.width/2,privacy.height/2); input.wait(40)
      test.check(manager.newRoomPrivate && bridge.sent.length===beforeCreate,"private creation choice remains local until Create")
      manager.close(); manager.createRoom(); input.wait(40)
      test.check(!privacy.checked,"new rooms default public each time")
      manager.close(); input.wait(40)
