            ...
            move || {
                ...
                async move {
                    ...
                    if let Err(error) = session.wait_pc_connection().await {
                        // A connected signaling session already owns native peer
                        // connections and background tasks. Dropping RtcSession
                        // does not close those resources on an ICE join failure.
                        session.close(DisconnectReason::JoinFailure).await;
                        return Err(error);
                    }
