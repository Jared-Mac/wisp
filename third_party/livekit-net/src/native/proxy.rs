...
pub(super) async fn connect_ws(
    ...
    let ws_stream = if let Ok(proxy_url) = proxy_env {
        if !proxy_url.is_empty() {
            ...
            let mut proxy_stream = crate::connect_tcp(proxy_host, proxy_port)
                .await
            ...
            let (ws_stream, _) = connect_direct(request, url).await?;
            ws_stream
        }
    } else {
...
async fn connect_direct(
    request: http::Request<()>,
    url: &url::Url,
) -> Result<(WebSocketStream<MaybeTlsStream<TcpStream>>, http::Response<Option<Vec<u8>>>), TransportError> {
    let host = url.host_str().ok_or_else(|| TransportError::Connection("Target URL has no host".into()))?;
    let port = url.port_or_known_default().ok_or_else(|| TransportError::Connection("Target URL has no port".into()))?;
    let socket = crate::connect_tcp(host, port).await.map_err(|error| TransportError::Connection(error.to_string()))?;
    // Preserve the original hostname for TLS verification, SNI and HTTP Host.
    client_async_tls_with_config(request, socket, None, None).await.map_err(map_ws_err)
}
