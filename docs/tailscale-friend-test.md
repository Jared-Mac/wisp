...
7. Select **Clear**, compare keyboard/fan/air-conditioner noise with **Studio**,
   ...
   only if the fallback was needed). Move the strength slider while speaking
   and confirm the change applies without reconnecting. Start near 25%, then
   raise it only as far as needed for the room noise. The queue should remain
   near zero, the late-frame counter should not steadily increase, and quiet
   speech should not lose its first or last syllable.
